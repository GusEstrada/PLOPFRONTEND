...
function InkPreview({ lines }: { lines: LineData[] }) {
  const uid = useId();
  ...
  return (
    <svg viewBox={`${minX - pad} ${minY - pad} ${w + pad * 2} ${h + pad * 2}`} className="w-full h-full">
...
function UserDetailModal({ drawing, onClose, currentUser, onNewAchievements, onBlotClick }: {
  drawing: Drawing;
  onClose: () => void;
      ...
      <div className="bg-[#FFFDF7] rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl border border-gray-200"
        ...
        <div className="p-6">
          <div className="w-full aspect-[4/3] bg-gray-100 rounded-xl overflow-hidden mb-4 relative">
            <InkPreview lines={drawing.lines} />
